...
def train(args, train_dataset, model, tokenizer):
    ...
    if os.path.exists(t_output_dir):
        try:
            ...
            steps_trained_in_current_epoch = (
                global_step * accum_step) % len(train_dataloader)
    ...
    for _ in train_iterator:  # epoch
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            global_step += 1
            if (
                args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0
            ):
            ...
            # save model
            if (
                args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0
            ):
...
def main():
    ...
    if (
        os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and not args.overwrite_output_dir
    ):
        raise ValueError(
    ...
    if args.do_train:
        ...
        if (os.path.isfile(
            ...
            # Load in optimizer and scheduler states
            model = model_class.from_pretrained(
                t_output_dir)  # , force_download=True)
            tokenizer = tokenizer_class.from_pretrained(args.output_dir)
            model.to(args.device)
            logger.info(
                " Loading model from %s [%s]", args.output_dir, t_output_dir)
        #########################################################################
